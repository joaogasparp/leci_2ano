...
while:  lw $t3,LATE($t1)        # obter configuracao de bits
        ...
        lw $t3,PORTB($t1)
        andi $t3,$t3,0x0008     # 0000 0000 0000 1000 -> PORTB3(RB3) a 1, ou seja, a ler o input RB3

if:     beq $t3,0x0008,else     # if(RB3 == 0x0008)
        bge $t0,0,elsen
        li $t0,15
        jendif

else:   blt $t0,15,elsep        # if($t0<15) salta para else
        li $t0,0                # cnt = 0
        j endif
